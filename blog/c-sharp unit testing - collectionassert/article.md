...
# C# unit testing - CollectionAssert

This is the third part of our journey. In the [first part](TODO) we listed the methods belonging to the **Assert** class, while in the [second part](TODO) we had a look at the **StringAssert** class. 
...
[This class](https://docs.microsoft.com/en-us/dotnet/api/microsoft.visualstudio.testtools.unittesting.collectionassert?f1url=https%3A%2F%2Fmsdn.microsoft.com%2Fquery%2Fdev15.query%3FappId%3DDev15IDEF1%26l%3DEN-US%26k%3Dk(Microsoft.VisualStudio.TestTools.UnitTesting.CollectionAssert);k(DevLang-csharp)%26rd%3Dtrue&view=mstest-net-1.2.0) tests various conditions associated with collections, like tests about the type of the elements. It is important to say that this class does not check if every element in the collection follows a certain rule (like "The ID must be greater than 0"), but it's more about high level checks.
...
Of course you should know that a AdminUser is also a User, but the vice versa is not true.
...
As you can see, the first test will pass, but the second one will fail because the object with `Name = "Freddie"` is the only object of type `AdminUser`.
...
Ok, it's not so amazing, but I wanted to show you what's under the hood.

So, let's get back to our problem: how can we make that test pass?

We must override the `Equals` method and, for correctness, also the `GetHashCode` method. Even here I reuse the `UpdatedUser` class from the first article 
...
[TestMethod]
public void AreEqual_RepeatedElements_Int()
{
    int[] array1 = new int[] { 1, 2, 2, 3, 4, 5, 6 };
    int[] array2 = new int[] { 1, 2, 3, 4, 4, 5, 6 };
    CollectionAssert.AreEqual(array1, array2); //KO
}
```
...
My favorite, by now, is [FluentAssertion](https://fluentassertions.com). It replaces the `Assert` class with a syntax that allows you to create more complex tests. It is more readable (well, fluent) and a test looks like this:
```csharp
[TestMethod]
...
So, MSTest is perfect if you are moving your first steps in the world of unit testing, but when the complexity grows you should consider to adopt other frameworks.
